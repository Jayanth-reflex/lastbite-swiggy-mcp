...
interface StepLike {
  ...
  toolResults?: Array<{ output?: unknown; result?: unknown; toolName?: string }>;
}
...
function extractRestaurantHints(result: ResultLike): TryParseCartOptions {
  const steps = result.steps ?? [];
  for (let s = steps.length - 1; s >= 0; s--) {
    const calls = steps[s].toolCalls ?? [];
    for (let c = calls.length - 1; c >= 0; c--) {
      const tc = calls[c];
      const argsRaw = tc.args ?? tc.input;
      if (!argsRaw || typeof argsRaw !== "object") continue;
      const args = argsRaw as Record<string, unknown>;
      const id = args.restaurantId ?? args.restaurant_id ?? args.restaurantIdOfAddedItem;
      if (id != null) return { restaurantIdHint: String(id) };
    }
  }
  return {};
}
...
/**
 * Walk steps in reverse, returning the first tool result that parses into
 * a Cart. Threads restaurantId hints from upstream tool-call args because
 * the cart payload itself doesn't include `restaurant.id`.
 */
export function coerceToCart(result: ResultLike): Cart | null {
  const hints = extractRestaurantHints(result);
  const steps = result.steps ?? [];
  for (let s = steps.length - 1; s >= 0; s--) {
    const trs = steps[s].toolResults ?? [];
    for (let t = trs.length - 1; t >= 0; t--) {
      const cart = tryParseCart(trs[t].output ?? trs[t].result, hints);
      if (cart) return cart;
    }
  }
  return null;
